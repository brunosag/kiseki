...
# ╔═╡ bb864cec-3040-4efa-bc68-5687d2049bdf
begin
	import Lux
	using LuxCUDA
	using Printf
	...
	using Random: rand!, Xoshiro, TaskLocalRNG
...
# ╔═╡ 5692ad87-2dd6-45b7-9b54-f43a23e36678
md"""
## `data.jl`
"""

# ╔═╡ 410a4a00-d470-4abd-adbc-5bc8d80b4ce7
begin
	struct BalancedDataLoader{T <: AbstractArray, V <: AbstractMatrix, R <: AbstractRNG}
	    X::T
	    Y::V
	    batchsize::Int
	    ...
	    function BalancedDataLoader(X::T, Y::V, y_raw::AbstractVector, batchsize::Int, rng::R) where {T <: AbstractArray, V <: AbstractMatrix, R <: AbstractRNG}
	        ...
	        class_groups = [findall(==(c), y_raw) for c in classes]
	        n_batches = minimum(length.(class_groups)) ÷ samples_per_class
	
	        batch_idx = Vector{Int}(undef, batchsize)
	
	        return new{T, V, R}(X, Y, batchsize, class_groups, samples_per_class, n_batches, batch_idx, rng)
	    end
	end
	...
	function Base.iterate(d::BalancedDataLoader)
	    for group in d.class_groups
	        shuffle!(d.rng, group)
	    end
	    return iterate(d, 1)
	end

	function Base.iterate(d::BalancedDataLoader, b::Int)
	    if b > d.n_batches
	        return nothing
	    end
	    ...
	    for group in d.class_groups
	        ...
	        copyto!(d.batch_idx, idx_offset, group, start_idx, count)
	        idx_offset += count
	    end
	
	    shuffle!(d.rng, d.batch_idx)
	
	    Xᵢ = @view d.X[:, :, :, d.batch_idx]
	    Yᵢ = @view d.Y[:, d.batch_idx]
	...
	function load_MNIST(rng::AbstractRNG, batchsize::Int; balanced::Bool = true)
	    ...
	    train_loader = balanced ? BalancedDataLoader(X_train, Y_train, train_data.targets, batchsize, rng) : DataLoader((X_train, Y_train); batchsize, rng, shuffle = true)
...
# ╔═╡ 28a608cf-6626-4a9b-9ea7-1546308bcbff
md"""
## `optimizers.jl`
"""

# ╔═╡ 8e7fe81d-7f53-42c6-b424-895c828fb3e4
abstract type AbstractOptimizer end

# ╔═╡ bac48b60-c73e-4397-ad02-165db85648db
abstract type AbstractOptimizerState end

# ╔═╡ b75c5033-9b94-4306-8fda-e5d48baf659b
md"""
## `optimizers/leea.jl`
"""

# ╔═╡ 04443613-7d6b-4376-8b66-47383282d5f9
const logitcrossentropy = Lux.CrossEntropyLoss(; logits = Val(true))

# ╔═╡ 37b66c5f-a845-49a4-8735-dc6f7daef192
...
# ╔═╡ 025671ca-0557-4929-b265-e0c2519e2514
...
# ╔═╡ a02d8a53-3cb4-43a4-afaf-a8b4fedea466
...
# ╔═╡ dc45a07f-3f94-43c4-974a-71a2e309db09
...
# ╔═╡ af7acbf4-8076-4fc2-8e0b-2682acbd2201
...
# ╔═╡ f4e6b5d2-e1a5-4bf2-b794-75ca6bfc3dc9
...
# ╔═╡ ee37e708-3651-46ab-8aa0-58a074537292
...
# ╔═╡ c97df2a7-517b-42cf-8e6d-3a6801176523
...
# ╔═╡ 546f882f-f1fb-4b89-8881-72edcd2a5861
function update_patience!(opt::LEEA, ops::LEEAState)
    ...
    return if ops.pat >= opt.pat_lim
        ops.m *= opt.γ
        ops.pat = 0
    end
end

# ╔═╡ 7adc17f1-f63e-41e9-bee4-299cf97bf160
...
# ╔═╡ eb5d52fa-85a9-4b6c-910d-155af38e0fa9
md"""
## `models.jl`
"""

# ╔═╡ af367470-bd0c-44ec-900f-15f76867ad0e
...
# ╔═╡ e8e6169f-35e0-40e3-ab3a-4c60bfae1599
md"""
## `experiment.jl`
"""

# ╔═╡ 3ca8c27a-cec9-4ea5-82e7-09066cff709f
...
# ╔═╡ a8f3a011-39fe-4d09-9b91-5fb8f8efbf7a
...
# ╔═╡ 96dcaa1b-a78f-4eba-804e-1ad3fe0bceb1
...
# ╔═╡ 30253caf-6842-42da-989e-c520907b368a
function run(exp::Experiment; est::Union{ExperimentState, Nothing} = nothing)
	...
	while est.i <= exp.max_i && est.best_acc < exp.target_acc
		...
		Δt = time() - t₀
		@info @sprintf "i = %-*d    Δt = %.2fs\n" ndigits(exp.max_i) est.i Δt

		est.i += 1
	end
end

# ╔═╡ aaae18f2-d95b-4693-a26f-7a45d7fadf81
md"""
## `train.jl`
"""

# ╔═╡ 5824937c-f100-4944-b9a7-507b79c933c9
exp = Experiment()

# ╔═╡ 61fd5493-acad-4b4e-922c-b26c21728c42
run(exp)
...
manifest_format = "2.0"
project_hash = "fc9c4089912098792650df5e67ebfd7e3d5643e6"
...
git-tree-sha1 = "1b4e9bf1c386ccd4815d00859d1a354562d98af2"
...
git-tree-sha1 = "f40fa78679f0d5584f6b83abed10e78c34d8bd9d"
