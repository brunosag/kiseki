...
const OPTIMIZERS = (
    sgd=[
        (key="η", label=raw"\eta", default=0.01, type="number", step=0.01, desc="Learning rate")
    ],
    leea=[
        (key="N", label=raw"N", default=200, type="number", step=1, desc="Population size"),
        (key="pₘ", label=raw"p_{\mathrm{m}}", default=0.04, type="number", step=0.01, desc="Mutation probability"),
        (key="η₀", label=raw"\eta_0", default=0.03, type="number", step=0.01, desc="Initial mutation step size"),
        (key="γ", label=raw"\gamma", default=0.99, type="number", step=0.01, desc="Mutation decay factor"),
        (key="ρ", label=raw"\rho", default=0.4, type="number", step=0.01, desc="Retention fraction"),
        (key="ρₓ", label=raw"\rho_{\mathrm{x}}", default=0.5, type="number", step=0.01, desc="Crossover fraction"),
        (key="λ", label=raw"\lambda", default=0.2, type="number", step=0.01, desc="Fitness decay coefficient"),
        (key="τ_pat", label=raw"\tau_{\mathrm{pat}}", default=25, type="number", step=1, desc="Validation patience threshold")
    ]
)

const STOP_SIGNAL = Threads.Atomic{Bool}(false)
const EXPERIMENT_RUNNING = Threads.Atomic{Bool}(false)
const ACTIVE_EST = Ref{Union{ExperimentState,Nothing}}(nothing)

function update_ui!(model, est)
    model.current_step[] = est.i
    model.best_acc[] = est.best_acc
    ...
    acc_y = [a.value for a in est.history.acc]
    ...
    @onchange start_experiment begin
        if start_experiment
            try
                ...
                kwargs = NamedTuple(
                    param.key => param.step == 1 ? round(Int, val) : Float64(val)
                    for param in optimizers_schema[Symbol(optimizer)]
                )
                opt = getproperty(Kiseki, Symbol(optimizer))(; kwargs...)
